...
> = {
  queued: { label: "Queued", class: "text-muted-foreground bg-muted border", icon: <SpinnerIcon className="w-3 h-3 text-muted-foreground" /> },
  running: { label: "Running", class: "text-info bg-info/5 border border-info/30", icon: <SpinnerIcon className="w-3 h-3 text-info" /> },
  awaiting_hitl: {
    label: "Needs review",
    class: "text-warning bg-warning/5 border border-warning/30",
    icon: <span className="w-1.5 h-1.5 rounded-full bg-warning" />,
  },
  completed: {
    label: "Completed",
    class: "text-success bg-success/5 border border-success/30",
    icon: <span className="w-1.5 h-1.5 rounded-full bg-success" />,
  },
  failed: {
    label: "Failed",
    class: "text-destructive bg-destructive/5 border border-destructive/30",
    icon: <span className="w-1.5 h-1.5 rounded-full bg-destructive" />,
  },
  cancelled: {
    label: "Cancelled",
    class: "text-muted-foreground bg-muted border",
    icon: <span className="w-1.5 h-1.5 rounded-full bg-muted-foreground" />,
  },
};
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-0.5 rounded-md ${cfg.class}`}
    >
      {cfg.icon}
      {cfg.label}
    </span>
...
const CompletedPipelineReviewPanel: React.FC<{
  ...
  <Panel className="mb-8 overflow-hidden text-left animate-fadeIn">
    <div className="border-b px-6 py-4 flex items-center justify-between">
      <div>
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-foreground">Completed Pipeline Review</h3>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Review the full path that led to the final report.
        </p>
      </div>
    </div>
...
export const PipelineView: React.FC<PipelineViewProps> = ({
  ...
  const renderFormattedThinking = (text: string) => {
    if (!text) return <span className="text-muted-foreground italic">No thinking recorded.</span>;
    return (
      <div className="space-y-2 leading-relaxed text-muted-foreground text-xs whitespace-pre-wrap font-sans">
        {text.split('\n').map((line, idx) => {
          const trimmed = line.trim();
          if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
            return (
              <div key={idx} className="flex gap-2 pl-2 text-muted-foreground">
          ...
          if (/^\d+\.\s/.test(trimmed)) {
            ...
            if (match) {
              return (
                <div key={idx} className="flex gap-2 pl-2 text-muted-foreground">
                  <span className="text-primary font-bold select-none">{match[1]}</span>
                  <span>{match[2]}</span>
                </div>
              );
            }
          ...
          if (trimmed.startsWith('### ')) {
            return (
              <h4 key={idx} className="text-xs font-bold text-foreground mt-2 mb-1">
                {trimmed.substring(4)}
              </h4>
            );
          }
          if (trimmed.startsWith('## ')) {
            return (
              <h3 key={idx} className="text-xs font-bold text-foreground mt-3 mb-2 border-b border-border pb-0.5">
                {trimmed.substring(3)}
              </h3>
            );
          }
  ...
  const thinkingKeys = [
    ...
    { key: "input_validator", label: "Input Validator Agent", icon: "" },
    { key: "planner", label: "Execution Planner Agent", icon: "" },
    { key: "validator", label: "Output Validator Agent", icon: "" },
  ];
  ...
  return (
    <div className="w-full h-full flex flex-col flex-1 min-h-0 text-left">
      {/* Sticky Run Strip */}
      <div className="sticky top-0 z-40 w-full flex-none flex flex-col gap-3 bg-card px-4 py-2.5 border-b md:flex-row md:items-center md:justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 border bg-muted/30 p-0.5 rounded-md">
            <button
                ...
                {activeLogTab === "anomalies" ? (
                  ...
                  <div className="p-4 space-y-4 flex-1">
                    {thinkingKeys.map(({ key, label, icon }) => {
                      ...
                      return (
                        <div key={key} className="border border-border bg-card rounded-lg overflow-hidden transition-all duration-200 hover:border-border/80">
                          <button
                            onClick={() => toggleAccordion(key)}
                            className="w-full px-4 py-2.5 flex items-center justify-between text-left hover:bg-muted/30 transition-colors cursor-pointer"
                          >
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-semibold text-foreground">{label}</span>
                              {isActive && (
                                <span className="flex items-center gap-1 text-[10px] bg-info/5 text-info border border-info/30 px-2 py-0.5 rounded-md font-semibold">
                                  <span className="w-1.5 h-1.5 rounded-full bg-info animate-pulse" />
                                  Thinking...
                                </span>
                              )}
                              {!isActive && hasThinking && (
                                <span className="text-[10px] bg-success/5 text-success border border-success/30 px-2 py-0.5 rounded-md font-semibold">
                                  Complete
                                </span>
                              )}
                              {!isActive && !hasThinking && (
                                <span className="text-[10px] bg-muted text-muted-foreground border px-2 py-0.5 rounded-md font-semibold">
                                  Pending
                                </span>
                              )}
                            </div>
                            <span className="text-[10px] text-muted-foreground font-mono transition-transform duration-200">
                              {isExpanded ? "▲" : "▼"}
                            </span>
                          </button>
                          
                          {isExpanded && (
                            <div className="px-4 pb-4 border-t border-border pt-3 bg-muted/5">
                              {isActive && !hasThinking ? (
                                <div className="flex items-center gap-2 py-4 text-muted-foreground justify-center font-sans text-xs">
                                  <SpinnerIcon className="w-3.5 h-3.5 text-info animate-spin" />
                                ...
                                <div className="py-3 text-muted-foreground text-[10px] italic text-center font-sans">

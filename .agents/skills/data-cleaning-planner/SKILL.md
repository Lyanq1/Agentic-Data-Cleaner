...
- **Adjust Parameters**: 
  ...
  - If type casting failed, check if you need to provide a custom `parse_format`, change the target `expected_type`, or let a previous task handle data cleaning first. 
...
- **Provide Rationale**: Update the `rationale` field of the failing task to explicitly state *how* you changed the plan to address the prior validation failure. Do not generate the exact same plan again!

---

### **STEP 5 — OUTPUT JSON SCHEMA FORMAT**

...
class ValidatorAgent(BaseAgent):
    ...
    async def run(self, state: GlobalState) -> dict[str, Any]:
        ...
        try:
            ...
            # Instruct the model to return JSON conforming to the ValidatorOutput schema.
            schema_str = json.dumps(ValidatorOutput.model_json_schema(), indent=2)
            messages.append(
                SystemMessage(
                    content=(
                        "CRITICAL: You must output ONLY a valid JSON object matching the following schema. "
                        "Do NOT wrap the response in markdown code blocks like ```json ... ```, and do NOT add any trailing characters or conversational text.\n\n"
                        f"Schema:\n{schema_str}"
                    )
                )
            )

            raw_response = await self.json_llm.ainvoke(messages)
            ...
            output = ValidatorOutput.model_validate_json(content_clean)

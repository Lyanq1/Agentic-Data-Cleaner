...
def perform_data_quality_check(
    ...
    allowed_disguised_values: dict[str, list[str]] | None = None,
) -> str:
    ...
    Args:
        ...
        allowed_disguised_values: Exact per-column sentinel values approved by the user.
    ...
    profiler = QualityProfiler(
        ...
        allowed_disguised_values=allowed_disguised_values,
    )
    report = profiler.check_file(file_path)
    
    return json.dumps(report.to_dict(), indent=2)

def perform_data_quality_check_df(
    df: pd.DataFrame,
    null_threshold: float = 0.0,
    allowed_disguised_values: dict[str, list[str]] | None = None,
) -> str:
